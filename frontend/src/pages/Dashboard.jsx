...
function normalize(item) {
  const ca = item?.candidate_analysis || {};
  const ml = item?.ml_prediction || {};

  const confidence = confidencePercent(ml.confidence);

  return {
    id: item?.id ?? item?.analysis_id,
    filename: item?.filename || "Unknown Target",
    target: item?.filename || "Unknown Target",

    period: num(ca.period_days),
    depthPercent: depthPercent(ca.transit_depth),
    duration: num(ca.transit_duration_days),

    transits: num(ca.number_of_transits),
    power: num(ca.bls_power),
    snr: num(ca.bls_snr),
    score: num(ca.candidate_score),
    ...
    lightCurve: curve(item?.light_curve),
    phaseFoldedCurve: phase(item?.phase_folded_curve),

    transitsData: transit(
      item?.detected_transits ??
      item?.transit_times ??
      item?.transit_data
    ),

    oddEvenDifference: num(ca.odd_even_difference),
    periodicityScore: num(ca.periodicity_score),
  };
}
function curve(raw) {
...
export default function Dashboard({ setPage, setSelectedAnalysisId }) {
  ...
  useEffect(() => { load() }, []);
  const filteredItems = useMemo(() => { if (range === 0) return items; const cutoff = Date.now() - range * 86400000; return items.filter(x => !x.created || new Date(x.created).getTime() >= cutoff) }, [items, range]);
  const sorted = useMemo(() => [...filteredItems].sort((a, b) => new Date(b.created || 0) - new Date(a.created || 0)), [filteredItems]);
  ...
  const avg = filteredItems.length ? filteredItems.reduce((s, x) => s + x.score, 0) / filteredItems.length : 0;
  const statsRows = [
  ...
  const lc = latest?.lightCurve || [];
  const pc = latest?.phaseFoldedCurve || [];
  const td = latest?.transitsData || [];
  ...
  return <div className="mx-auto max-w-[1280px] px-5 py-6 xl:px-7">
    <header className="mb-5 flex items-start justify-between">
      ...
      <div className="flex items-center gap-2"><button onClick={load} disabled={loading} className="flex items-center gap-2 rounded-lg border border-white/[.08] bg-[#07101f] px-3 py-2.5 text-[11px] text-slate-300"><RefreshCw size={13} className={loading ? "animate-spin" : ""} />Refresh</button><select value={range} onChange={e => setRange(Number(e.target.value))} className="rounded-lg border border-white/[.08] bg-[#07101f] px-3 py-2.5 text-[11px] text-slate-300 outline-none"><option value={7}>Last 7 Days</option><option value={30}>Last 30 Days</option><option value={90}>Last 90 Days</option><option value={0}>All Time</option></select></div>
    </header>
    ...
    <section className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1fr)_318px]">
      <div className="dr-card rounded-xl p-2.5">
        ...
        <div className="mt-2 grid grid-cols-6 gap-1.5">
          ...
          <Metric label="Transit Depth" value={latest?.depthPercent ? `${latest.depthPercent.toFixed(3)}%` : "—"} tone="violet" />
    ...
    <section className="dr-card mt-3 overflow-hidden rounded-xl">
      <div className="flex items-center justify-between border-b border-white/[.07] px-5 py-4"><h2 className="text-[16px] font-semibold text-white">Top Planetary Candidates</h2><button onClick={() => setPage?.("candidates")} className="flex items-center gap-2 rounded-lg bg-[#10192f] px-3 py-2 text-[11px] text-slate-300">View All Candidates <ArrowRight size={13} /></button></div>
      <div className="overflow-x-auto"><table className="w-full min-w-[850px]"><thead><tr><th className="table-head">Rank</th><th className="table-head">Target</th><th className="table-head">Period (days)</th><th className="table-head">Score</th><th className="table-head">Confidence</th><th className="table-head">Priority</th><th className="table-head">Analyzed</th></tr></thead>
        <tbody>{unique.length === 0 ? <tr><td colSpan="7" className="p-10 text-center text-xs text-slate-600">No analyses in the selected period.</td></tr> : unique.slice(0, 5).map((x, i) => <tr key={x.id || x.filename} className="border-t border-white/[.05] hover:bg-white/[.018]"><td className="px-5 py-3 text-xs text-slate-400"><span className={`inline-flex h-6 w-6 items-center justify-center rounded-full ${i === 0 ? "bg-amber-400 text-slate-950" : i === 1 ? "bg-slate-300 text-slate-900" : i === 2 ? "bg-orange-500/70 text-white" : "text-slate-400"}`}>{i + 1}</span></td><td className="px-3 py-3 text-[12px] text-slate-200"><button onClick={() => { setSelectedAnalysisId?.(x.id); setPage?.("analyze") }} className="text-left hover:text-violet-300">{x.filename}</button></td><td className="px-3 py-3 text-[12px] text-slate-300">
          {Number(x.period || 0).toFixed(3)}
        </td><td className="px-3 py-3 text-[12px] font-semibold text-white">{x.score ? x.score.toFixed(1) : "—"}%</td><td className="px-3 py-3 text-[12px] text-slate-200">{Number(x.confidence || 0).toFixed(1)}%</td><td className="px-3 py-3"><span className={`rounded-md px-2 py-1 text-[9px] font-semibold ${x.confidence >= 80 ? "bg-emerald-500/10 text-emerald-300" : x.confidence >= 65 ? "bg-amber-500/10 text-amber-300" : "bg-blue-500/10 text-blue-300"}`}>{x.confidence >= 80 ? "HIGH" : x.confidence >= 65 ? "MEDIUM" : "LOW"}</span></td><td className="px-3 py-3 text-[11px] text-slate-500">{x.created ? new Date(x.created).toLocaleString() : "—"}</td></tr>)}</tbody></table></div>
    </section>
  </div>
}
